...
const answerValidation = (data) => {
  ...
  const correctAnswerController=expressAsyncHandler(async (req,res)=>{
      ...
      if (proposition && answerCandidate){
         if ( answerCandidate.response ==proposition.veracity){ 
            ...
            const updateVoucher= await Voucher.findOneAndUpdate({_id:voucher},{$set:{score:score}},
            {new: true}, ) 
  ...
  module.exports={answerValidation,addAnswerController,getAnswerController,correctAnswerController}