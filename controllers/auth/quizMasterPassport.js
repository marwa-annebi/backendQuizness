...
passport.use(
    ...
    User.findOne({linkedinId: profile.id }).then( async function(currentUser)  {
      if (currentUser) {
          if (!currentUser.isQuizmaster){
            await User.findOneAndUpdate({linkedinId:profile.id,isQuizmaster:false},{$set:{isQuizmaster:true}},
                {new: true}, ) 
          }
        console.log("user is:", currentUser);
        done(null, currentUser);
    ...
    new MicrosoftStrategy(
      {
        ...
        callbackURL: "http://localhost:5000/auth/microsoft/callbackQuizmaster",
